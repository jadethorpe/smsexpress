...
class smsExpress(toga.App):
    ...
    def config_function(self, widget):
        self.label_title  = toga.Label('Enter the Twilio configuration information that was provided by your system administrator')
        self.label_account_id = toga.Label('Account ID', style=Pack(padding_top=20, padding_bottom=10))
        self.input_account_id = toga.MultilineTextInput(id='input1', initial=self.creds["account_sid"])
        self.label_api_key  = toga.Label('API KEY', style=Pack(padding_top=20, padding_bottom=10))
        self.input_api_key = toga.MultilineTextInput(id='input2', initial=self.creds["api_key"])
        self.label_api_secret = toga.Label('API SECRET', style=Pack(padding_top=20, padding_bottom=10))
        self.input_api_secret = toga.MultilineTextInput(id='input3', initial=self.creds["api_secret"])
        self.label_service_id = toga.Label('Service ID', style=Pack(padding_top=20, padding_bottom=10))
        self.input_service_id = toga.MultilineTextInput(id='input4', initial=self.creds["service_id"], style=Pack(padding_bottom=20))
        btn_save = toga.Button("Save", 
                on_press=self.make_config, 
                style=Pack(flex=1))
    ...
    def action_question_dialog(self, widget):
        if self.main_window.question_dialog(
            "smsExpress", "Ready to send message to distribution list?"
        ):
            self.label.text = "Sending to Recipient List!"
            self.main_window.info_dialog("Sending Message", self.textbox.value)
            self.progress.start()
    ...
    def action_error_dialog(self, widget):
        self.main_window.error_dialog("smsExpress", "Well that didn't work...")
        self.label.text = "Oh noes..."

    def action_config_dialog(self, widget):
        self.main_window.error_dialog("smsExpress", "Please configure Twilio Settings First")
        self.label.text = "Oh noes..."
    ...
    def make_config(self, widget):
        print("Make Config Called with the following params")
        print(self.input_account_id.value)
        print(self.input_api_key.value)
        print(self.input_api_secret.value)
        print(self.input_service_id.value)
        self.creds = dict()
    ...
    def startup(self):
        ...
        #self.creds = self.load_config()
        # Set up main window
        self.main_window = toga.MainWindow(title=self.name)
        self.label_account_id = toga.Label('Account ID', style=Pack(padding_top=20, padding_bottom=10))
        self.input_account_id = toga.TextInput(id='input1', initial=self.creds["account_sid"])
        self.label_api_key  = toga.Label('API KEY', style=Pack(padding_top=20, padding_bottom=10))
        self.input_api_key = toga.TextInput(id='input2', initial=self.creds["api_key"])
        self.label_api_secret = toga.Label('API SECRET', style=Pack(padding_top=20, padding_bottom=10))
        self.input_api_secret = toga.TextInput(id='input3', initial=self.creds["api_secret"])
        self.label_service_id = toga.Label('Service ID', style=Pack(padding_top=20, padding_bottom=10))
        self.input_service_id = toga.TextInput(id='input4', initial=self.creds["service_id"], style=Pack(padding_bottom=20))
        ...
        cmd_config = toga.Command(self.config_function, label='Config', group=config_group) 
    ...
    def send_sms(self, message, input_file):
        ...
        for recipient in recipients:
            if recipient["phone_number"]:
                ...
                print(phone_number)
                print(Message)
                recipient_list.append(f'{{"binding_type": "sms", "address": "{phone_number}" }}')
            else:
                pass
            count += 1
        ...
        notification = self.client.notify.services(self.service_id).notifications.create(
            to_binding = recipient_list,
            body = Message)
